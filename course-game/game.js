...
const serinFallback = document.getElementById("serinFallback");
...
function triggerSerinPopupOnce() {
    if (isSerinPopupTriggered) {
        return;
    }

    const randomDelay = Math.floor(Math.random() * 30000) + 10000;

    serinPopupTimer = setTimeout(function () {
...
function clearSerinPopupTimer() {
    clearTimeout(serinPopupTimer);
    serinPopupTimer = null;
}

function startRandomSerinPopup() {
    setSerinImage("Serin_01.png");
    serinBtnGroup.style.display = "flex";
    serinModal.style.display = "flex";
}

function setSerinImage(src) {
    serinFallback.classList.remove("is-visible");
    serinMainImg.classList.remove("is-hidden");
    serinMainImg.src = src;
}

function showSerinResult(src) {
    setSerinImage(src);
    serinBtnGroup.style.display = "none";
}

function closeSerinPopup() {
    serinModal.style.display = "none";
}

serinMainImg.addEventListener("error", function () {
    serinMainImg.classList.add("is-hidden");
    serinFallback.classList.add("is-visible");
});

trustBtn.addEventListener("click", function () {
    showSerinResult("Serin_03.png");
});

distrustBtn.addEventListener("click", function () {
    showSerinResult("Serin_02.png");
});

serinCloseX.addEventListener("click", closeSerinPopup);
...
function openSabotagedMacroGimmick() {
    ...
    macroSabotageHandler = function () {
        ...
        let fakeChar = randomChars[Math.floor(Math.random() * randomChars.length)];

        if (fakeChar === secondChar) {
            fakeChar = randomChars[(randomChars.indexOf(fakeChar) + 1) % randomChars.length];
        }
